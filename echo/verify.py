...
def _compute_similarity_confidence(
    overlap_score: float, jaccard_score: float, edit_density: float
) -> float:
...
def verify_near_miss(
    ...
    if not tokens1 or not tokens2:
        ...
        return VerificationResult(
            ...
            overlap_score=0.0,
            jaccard_score=0.0,
            edit_distance=max(len(tokens1), len(tokens2)),
            edit_density=1.0,
            lcs_length=0,
            token_diff_count=max(len(tokens1), len(tokens2)),
    ...
    if overlap_score >= overlap_threshold:
        ...
        is_duplicate = True
        decision_reason = (
            f"High confidence: {similarity_confidence:.3f} >= {min_confidence}"
        )
    else:
    ...
    logger.debug(
        f"Verification: {block1.original.file_path}:{block1.original.start_line} vs "
...
def batch_verify(
    target_block: NormalizedBlock,
    candidate_blocks: List[NormalizedBlock],
    config: Optional[EchoConfig] = None,
) -> List[Tuple[NormalizedBlock, VerificationResult]]:
    ...
    logger.debug(
        f"Batch verifying {len(candidate_blocks)} candidates against target "
    ...
    for candidate in candidate_blocks:
        ...
        if (
            candidate.original.file_path == target_block.original.file_path
            and candidate.original.start_line == target_block.original.start_line
        ):
            continue
    ...
    logger.info(
        f"Verified {duplicate_count}/{verification_count} candidates as duplicates "
...
def verify_candidate_pairs(
    candidate_pairs: List[Tuple[NormalizedBlock, NormalizedBlock]],
    config: Optional[EchoConfig] = None,
) -> List[Tuple[NormalizedBlock, NormalizedBlock, VerificationResult]]:
    ...
    logger.info(
        f"Verified {duplicate_count}/{len(candidate_pairs)} pairs as duplicates"
    )
...
def get_verification_statistics(results: List[VerificationResult]) -> Dict[str, float]:
    ...
    if duplicate_results:
        stats.update(
            {
                "avg_overlap_score_duplicates": sum(
                    r.overlap_score for r in duplicate_results
                )
                / len(duplicate_results),
                "avg_jaccard_score_duplicates": sum(
                    r.jaccard_score for r in duplicate_results
                )
                / len(duplicate_results),
                "avg_edit_density_duplicates": sum(
                    r.edit_density for r in duplicate_results
                )
                / len(duplicate_results),
                "avg_confidence_duplicates": sum(
                    r.similarity_confidence for r in duplicate_results
                )
                / len(duplicate_results),
            }
        )
