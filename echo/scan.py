...
from typing import List, Dict, Iterator, Optional, Set, Tuple, Any
...
from .normalize import normalize_blocks, NormalizedBlock
from .lsh import MinHashLSH, create_lsh_from_config
from .verify import verify_near_miss, VerificationResult
from .embed import compute_semantic_similarity, SemanticEmbedder
from .storage import EchoDatabase, DuplicateFinding, BlockRecord, create_database
...
class DuplicateScanner:
    ...
    def __init__(self, config: EchoConfig, database: EchoDatabase, 
        ...
        logger.info(f"Initialized DuplicateScanner with config: "
    ...
    def scan_repository(self, repo_path: Path, 
                       budget_ms: Optional[int] = None) -> ScanResult:
        ...
        try:
            ...
            findings = self._run_detection_pipeline(all_blocks, stats, budget_ms, start_time)
            
            stats.scan_time_ms = int((time.time() - start_time) * 1000)
            logger.info(f"Repository scan completed in {stats.scan_time_ms}ms. "
    ...
    def scan_changed_files(self, file_paths: List[Path]) -> ScanResult:
        ...
        try:
            ...
            stats.stage_times_ms['index_update'] = int((time.time() - stage_start) * 1000)
            
            stats.scan_time_ms = int((time.time() - start_time) * 1000)
            logger.info(f"Incremental scan completed in {stats.scan_time_ms}ms. "
                       f"Found {len(findings)} duplicate findings.")
    ...
    def _extract_repository_blocks(self, repo_path: Path, stats: ScanStatistics) -> List[NormalizedBlock]:
    ...
    def _extract_blocks_from_files(self, file_paths: List[Path], stats: ScanStatistics) -> List[NormalizedBlock]:
        ...
        for file_path in file_paths:
            try:
                ...
                for block in raw_blocks:
                    if (block.token_count >= self.config.min_tokens and 
                        block.lines > 1):  # Suppress single-line matches
        ...
        logger.info(f"Extracted {len(all_blocks)} normalized blocks from {stats.files_processed} files")
        return all_blocks
    
    def _update_index_with_blocks(self, blocks: List[NormalizedBlock], stats: ScanStatistics) -> None:
    ...
    def _run_detection_pipeline(self, blocks: List[NormalizedBlock], stats: ScanStatistics,
                               budget_ms: Optional[int] = None, start_time: Optional[float] = None) -> List[DuplicateFinding]:
        ...
        try:
            ...
            for i, block in enumerate(blocks):
                ...
                if budget_ms and start_time:
                    ...
                    if elapsed_ms > budget_ms * 0.95:
                        logger.warning(f"Approaching time budget at block {i}/{len(blocks)}")
                        break
            ...
            stats.stage_times_ms['final_filter'] = int((time.time() - stage_start) * 1000)
            
            logger.info(f"Pipeline generated {len(findings)} findings, filtered to {len(filtered_findings)}")
    ...
    def _process_block(self, block: NormalizedBlock, stats: ScanStatistics) -> List[DuplicateFinding]:
        ...
        try:
            ...
            verified_candidates = self._verify_near_miss_candidates(block, candidates, stats)
            
            # Stage 3: Semantic reranking (if enabled)
            if self.config.tau_semantic > 0 and self.semantic_embedder:
                verified_candidates = self._semantic_rerank_candidates(block, verified_candidates, stats)
            
            # Convert to findings and apply per-block filtering
            block_findings = self._create_findings_from_candidates(block, verified_candidates, stats)
            
        except Exception as e:
            logger.warning(f"Failed to process block {block.original.file_path}:{block.original.start_line}: {e}")
            stats.errors += 1
            
        return block_findings
    
    def _generate_lsh_candidates(self, block: NormalizedBlock, stats: ScanStatistics) -> List[str]:
        ...
        try:
            ...
            candidates = self.lsh.query_candidates(block, topk=self.config.max_candidates)
            stats.lsh_queries += 1
    ...
    def _verify_near_miss_candidates(self, block: NormalizedBlock, candidates: List[str], 
                                   stats: ScanStatistics) -> List[Tuple[str, VerificationResult]]:
        ...
        for candidate_id in candidates:
            try:
                ...
                logger.warning(f"Near-miss verification failed for candidate {candidate_id}: {e}")
                stats.errors += 1
                
        return verified
    
    def _semantic_rerank_candidates(self, block: NormalizedBlock, 
        ...
        for candidate_id, verification_result in verified_candidates:
            try:
                ...
                logger.warning(f"Semantic reranking failed for candidate {candidate_id}: {e}")
                stats.errors += 1
                
        return semantic_verified
    
    def _create_findings_from_candidates(self, block: NormalizedBlock, 
                                       verified_candidates: List[Tuple[str, VerificationResult]],
                                       stats: ScanStatistics) -> List[DuplicateFinding]:
        ...
        for candidate_id, verification_result in verified_candidates:
            try:
                ...
                if hasattr(verification_result, 'semantic_similarity'):
                    scores['semantic_similarity'] = verification_result.semantic_similarity
                ...
                finding = DuplicateFinding(
                    ...
                    scores=scores,
                    type='near_miss' if not hasattr(verification_result, 'semantic_similarity') else 'semantic',
    ...
    def _final_filter_and_rank(self, findings: List[DuplicateFinding], 
                             stats: ScanStatistics) -> List[DuplicateFinding]:
        ...
        filtered_findings = [
            f for f in findings 
            if f.refactor_score >= self.config.min_refactor_score
        ]
    ...
    def _compute_refactor_score(self, block: NormalizedBlock, duplicate_count: int) -> float:
        ...
        score = block.token_count * (1 + math.log1p(duplicate_count)) * dispersion_factor
        return score
    ...
    def _load_candidate_block(self, candidate_id: str) -> Optional[NormalizedBlock]:
        ...
        try:
            ...
            block_record = self.database.session.query(BlockRecord).filter_by(
            ...
            if block_record:
                ...
                normalized_block = NormalizedBlock(
                    original=raw_block,
                    normalized_tokens=block_record.normalized_tokens.split() if block_record.normalized_tokens else [],
...
def scan_repository(repo_path: Path, config: Optional[EchoConfig] = None,
    ...
    try:
        ...
        logger.info(f"Repository scan completed: {len(result.findings)} findings in {result.statistics.scan_time_ms}ms")
        return result
...
def scan_changed_files(file_paths: List[Path], 
    ...
    try:
        ...
        logger.info(f"Incremental scan completed: {len(result.findings)} findings in {result.statistics.scan_time_ms}ms")
        return result
...
def scan_files_batch(file_paths: List[Path],
                    config: Optional[EchoConfig] = None,
                    max_workers: int = 4) -> ScanResult:
    ...
    try:
        ...
        logger.info(f"Batch scan completed: {len(all_findings)} findings in {stats.scan_time_ms}ms")
...
def create_scanner(config: Optional[EchoConfig] = None, 
                  db_path: Optional[Path] = None) -> DuplicateScanner:
...
def scan_with_semantic_reranking(repo_path: Path, 
                                config: Optional[EchoConfig] = None,
                                embedder: Optional[SemanticEmbedder] = None) -> ScanResult:
    ...
    try:
        ...
        logger.info(f"Semantic-enhanced scan completed: {len(result.findings)} findings")
        return result
    finally:
        database.session.close()