...
from .report import ReportConfig, generate_json_report, generate_markdown_report
from .scan import ScanResult, scan_changed_files, scan_repository
        ...
        try:
            for path in paths:
                ...
                try:
                    ...
                    click.echo(
                        f"✓ Indexed {path}: {result.get('files_indexed', 0)} files, "
...
@click.option(
    "--json", "json_output", type=click.Path(path_type=Path), help="JSON output file"
)
@click.option(
    "--markdown", type=click.Path(path_type=Path), help="Markdown output file"
)
@click.option(
    "--budget-ms", type=int, default=20000, help="Time budget in milliseconds"
)
@click.option(
    "--min-tokens", type=int, help="Minimum tokens per block (overrides config)"
)
@click.option(
    "--tau-sem", type=float, help="Semantic similarity threshold (overrides config)"
)
        ...
        try:
            ...
            if changed:
                ...
                result = scan_changed_files(
                    changed_files, config=config, progress_callback=progress_callback
                )
            else:
...
def status(ctx: click.Context) -> None:
    ...
    try:
        ...
        try:
            ...
            from .storage import BlockRecord, FileRecord, FindingRecord
            ...
            recent_files = (
                session.query(FileRecord.path, FileRecord.indexed_at)
...
def _print_scan_summary(result: ScanResult, verbose: bool) -> None:
    ...
    if findings:
        ...
        top_findings = sorted(
            findings, key=lambda f: f.scores.get("R", 0), reverse=True
        )[:3]
        ...
        for i, finding in enumerate(top_findings, 1):
            ...
            click.echo(
                f"  {i}. Refactor Score: {refactor_score:.1f}, "
