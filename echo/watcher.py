...
from typing import Callable, List, Optional, Set, Dict, Deque
...
class EventBatcher:
    ...
    def __init__(self, callback: Callable[[List[FileChangeEvent]], None], 
                 batch_delay: float = 2.0, max_batch_size: int = 100):
...
class EchoFileHandler(FileSystemEventHandler):
    ...
    def __init__(self, config: EchoConfig, indexer: RepositoryIndexer,
    ...
    def _should_process_event(self, file_path: Path) -> bool:
        ...
        if file_path.name.startswith('.') or file_path.suffix in {'.tmp', '.bak', '.swp'}:
            return False
    ...
    def _check_rate_limit(self) -> bool:
        ...
        if recent_events > self.max_events_per_minute:
            logger.warning(f"Rate limit exceeded: {recent_events} events in last minute")
            return False
...
class FileWatcher:
    ...
    def start_watching(self, watch_paths: List[Path], 
        ...
        try:
            ...
            self.handler = EchoFileHandler(self.config, self.indexer, self._on_files_changed)
            ...
            logger.info(f"File watcher started monitoring {len(valid_paths)} directories")
            return True
    ...
    def _on_files_changed(self, changes: List[FileChangeEvent]) -> None:
        ...
        if files_to_index:
            try:
                ...
                if result.errors:
                    logger.warning(f"Indexing errors: {result.errors[:3]}...")  # Show first 3 errors
        ...
        for change in moved_changes:
            try:
                ...
                if change.old_path:
                    ...
                    logger.debug(f"Processed file move: {change.old_path} -> {change.path}")
            except Exception as e:
                logger.error(f"Error processing file move {change.old_path} -> {change.path}: {e}")
    ...
    def get_status(self) -> Dict:
        ...
        if self.handler:
            ...
            status.update({
                ...
                'last_activity': handler_stats.last_activity.isoformat() if handler_stats.last_activity else None
            })
...
async def watch_repository(repo_path: Path, config: Optional[EchoConfig] = None,
