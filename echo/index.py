...
from .parser import extract_blocks, CodeBlock, detect_language
...
class RepositoryIndexer:
        ...
        if gitignore_path.exists():
            try:
                with open(gitignore_path, 'r', encoding='utf-8') as f:
                    git_patterns = [line.strip() for line in f if line.strip() and not line.startswith('#')]
        ...
        if dupesignore_path.exists():
            try:
                with open(dupesignore_path, 'r', encoding='utf-8') as f:
                    dupes_patterns = [line.strip() for line in f if line.strip() and not line.startswith('#')]
                    patterns.extend(dupes_patterns)
                    logger.debug(f"Loaded {len(dupes_patterns)} patterns from .dupesignore")
            except Exception as e:
    ...
    def index_repository(self, repo_path: Path, reindex: bool = False) -> IndexingResult:
        ...
        try:
            ...
            for i in range(0, len(files_to_process), batch_size):
                ...
                if i % (batch_size * 5) == 0:
                    ...
                    logger.info(f"Indexing progress: {progress}% ({i + batch_size}/{len(files_to_process)} files)")
    ...
    def _should_skip_directory(self, dir_path: Path) -> bool:
        ...
        skip_dirs = {'.git', '.svn', '.hg', '__pycache__', 'node_modules', 
                    '.idea', '.vscode', 'coverage', '.nyc_output', 'build', 'dist'}
                ...
                if blocks:
                    ...
                    for file_path in lang_files:
                        try:
                            ...
                            # Store file record
                            self.database.store_file(file_path, language, file_hash, file_size, lines)
                            ...
                            errors.append(f"File processing failed for {file_path}: {e}")
                    ...
                    for original, normalized in zip(blocks, normalized_blocks):
                        try:
                            ...
                            complexity = len(normalized.tokens) / 10.0  # Simple complexity metric
    ...
    def _file_needs_reindex(self, file_path: Path) -> bool:
        ...
        try:
            ...
            with self.database.get_session() as session:
                ...
                record = session.query(FileRecord).filter_by(
    ...
    def _compute_normalized_hash(self, normalized_block: NormalizedBlock) -> str:
        ...
        return hashlib.sha256(token_string.encode('utf-8')).hexdigest()[:16]  # 16 chars sufficient
    ...
    def get_changed_files(self, repo_path: Path) -> List[Path]:
        ...
        try:
            ...
            logger.warning(f"Git command failed: {e}, using file modification comparison")
...
def index_repository(repo_path: Path, config: Optional[EchoConfig] = None,
                    reindex: bool = False) -> IndexingResult:
