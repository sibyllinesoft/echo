...
class MinHashLSH:
    ...
    def add_block(self, block: NormalizedBlock) -> MinHashSignature:
        ...
        with self._lock:
            block_id = (
                block.original.file_path.as_posix() + f":{block.original.start_line}"
            )
            ...
            return MinHashSignature(
                block_id=block_id, signature=signature, shingles=shingles
            )
    ...
    def _persist_signature(
        self, block_id: str, signature: np.ndarray, block: NormalizedBlock
    ):
    ...
    def query_candidates(
        self, block: NormalizedBlock, max_candidates: int = 50
        ...
        with self._lock:
            for block in blocks:
                try:
                    ...
                    print(
                        f"Warning: Failed to add block {block.original.file_path}:{block.original.start_line}: {e}"
                    )
                    continue
        return signatures
    ...
    def load_from_database(self):
        ...
        try:
            ...
            with self._lock:
                ...
                blocks = (
                    self.db_session.query(BlockRecord)
                    .filter(BlockRecord.norm_hash.isnot(None))
                    .all()
                )
                ...
                for block_record in blocks:
                    try:
                        ...
                        print(
                            f"Warning: Failed to load signature for {block_record.id}: {e}"
                        )
                        continue
    ...
    def get_statistics(self) -> Dict[str, int]:
        ...
        with self._lock:
            ...
            total_buckets = sum(
                len(band_buckets) for band_buckets in self.buckets.values()
            )
            ...
            avg_bucket_size = (
                sum(bucket_sizes) / len(bucket_sizes) if bucket_sizes else 0
            )
