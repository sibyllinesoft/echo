...
try:
    ...
    raise ImportError("Tree-sitter language bindings are required. Install with: pip install tree-sitter-python tree-sitter-javascript tree-sitter-typescript")
...
class LanguageParser:
    ...
    def _setup_language(self) -> None:
        ...
        try:
            if self.language == 'python':
                lang = Language(ts_python.language(), "python")
            elif self.language == 'javascript':
                lang = Language(ts_javascript.language(), "javascript")
            elif self.language == 'typescript':
                lang = Language(ts_typescript.language(), "typescript")
            else:
    ...
    def extract_blocks(self, content: str, file_path: Path) -> List[CodeBlock]:
        ...
        try:
            ...
            for block in self._extract_structured_blocks(root_node, content, file_path, content_lines):
                blocks.append(block)
            ...
            chunk_blocks = self._extract_chunks(root_node, content, file_path, content_lines, extracted_ranges)
    ...
    def _extract_structured_blocks(self, root_node: Node, content: str, 
                                   file_path: Path, content_lines: List[str]) -> Iterator[CodeBlock]:
        ...
        def traverse(node: Node) -> Iterator[CodeBlock]:
            if node.type in extractable_types:
                ...
                if node.type not in {'program', 'module'} or self._is_meaningful_block(node):
                    block = self._create_code_block(node, content, file_path, content_lines)
    ...
    def _extract_chunks(self, root_node: Node, content: str, file_path: Path,
                       content_lines: List[str], extracted_ranges: Set[Tuple[int, int]]) -> List[CodeBlock]:
        ...
        for stmt in statements:
            ...
            if any(start <= stmt.start_byte < end or start < stmt.end_byte <= end 
            ...
            if current_tokens + len(stmt_tokens) > self.MAX_CHUNK_TOKENS and current_chunk_statements:
                chunk = self._create_chunk_block(current_chunk_statements, content, file_path, content_lines)
                if chunk:
                    chunks.append(chunk)
                current_chunk_statements = []
                current_tokens = 0
    ...
    def _find_statement_nodes(self, root_node: Node) -> List[Node]:
        ...
        def traverse(node: Node) -> None:
            ...
            statement_types = {
                ...
                'javascript': {
                    'expression_statement', 'variable_declaration', 'if_statement',
                    'for_statement', 'for_in_statement', 'while_statement', 'do_statement',
                    'try_statement', 'throw_statement', 'return_statement', 'break_statement',
                    'continue_statement', 'switch_statement', 'labeled_statement'
                },
                'typescript': {
                    'expression_statement', 'variable_declaration', 'if_statement',
                    'for_statement', 'for_in_statement', 'while_statement', 'do_statement',
    ...
    def _create_code_block(self, node: Node, content: str, file_path: Path,
                          content_lines: List[str]) -> Optional[CodeBlock]:
    ...
    def _create_chunk_block(self, statements: List[Node], content: str, file_path: Path,
                           content_lines: List[str]) -> Optional[CodeBlock]:
    ...
    def _extract_tokens(self, node: Node) -> List[str]:
        ...
        def traverse(n: Node) -> None:
            if n.type in {'identifier', 'string', 'number', 'true', 'false', 'null', 'none'}:
...
def extract_blocks(file_paths: List[Path], 
                   languages: Optional[List[str]] = None) -> List[CodeBlock]:
    """Extract code blocks from multiple files."""
    if not file_paths:
        return []
    
    supported_languages = set(languages) if languages else {'python', 'typescript', 'javascript'}
    blocks = []
...
def extract_blocks_from_content(content: str, file_path: Path, language: str) -> List[CodeBlock]:
