...
from .storage import BlockRecord, DuplicateFinding, EchoDatabase, FileRecord
...
class MarkdownReporter:
    ...
    def _write_summary(self, f: TextIO, scan_result: ScanResult) -> None:
        ...
        if stats.stage_times_ms:
            ...
            for stage, time_ms in sorted(stats.stage_times_ms.items()):
                ...
                f.write(
                    f"| {stage.replace('_', ' ').title()} | {time_ms:,} | {percentage:.1f}% |\n"
                )
            f.write("\n")
    ...
    def _write_findings(self, f: TextIO, findings: List[DuplicateFinding]) -> None:
        ...
        if not filtered_findings:
            f.write(f"## 🔍 Findings\n\n")
            f.write(
                f"ℹ️  {len(findings)} findings found, but none meet the minimum refactor score threshold of {self.config.min_refactor_score}.\n\n"
            )
            return

        f.write(
            f"## 🔍 Findings ({len(filtered_findings)} of {len(findings)} shown)\n\n"
        )
    ...
    def _group_by_folder(
        self, findings: List[DuplicateFinding]
    ) -> Dict[str, List[DuplicateFinding]]:
            ...
            if self.database:
                try:
                    ...
                    block1 = (
                        ...
                        .filter(BlockRecord.id == finding.block_id)
                        .first()
                    )

                    if block1 and block1.file_id:
                        file_record = (
                            self.database.session.query(FileRecord)
                            .filter(FileRecord.id == block1.file_id)
                            .first()
                        )
                    ...
                    logger.debug(
                        f"Could not determine folder for finding {finding.id}: {e}"
                    )
    ...
    def _write_finding_group(self, f: TextIO, findings: List[DuplicateFinding]) -> None:
        ...
        if self.config.sort_by_score:
            findings = sorted(
                findings, key=lambda x: x.scores.get("R", 0), reverse=True
            )
    ...
    def _write_finding(self, f: TextIO, finding: DuplicateFinding, index: int) -> None:
        ...
        other_scores = {
            k: v for k, v in finding.scores.items() if k not in ["R", "semantic"]
        }
        if other_scores:
    ...
    def _write_code_comparison(self, f: TextIO, finding: DuplicateFinding) -> None:
        ...
        if not self.database:
            f.write(
                "**Code Blocks:** _Database connection required to display code_\n\n"
            )
            return
    ...
    def _get_block_content(self, block_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            block = (
                self.database.session.query(BlockRecord)
                .filter(BlockRecord.id == block_id)
                .first()
            )
            ...
            if block.file_id:
                file_record = (
                    self.database.session.query(FileRecord)
                    .filter(FileRecord.id == block.file_id)
                    .first()
                )
            ...
            if file_record and Path(file_record.path).exists():
                try:
                    with open(file_record.path, "r", encoding="utf-8") as file:
                        ...
                        start_line = max(
                            0, block.start_line - self.config.context_lines - 1
                        )
                        end_line = min(
                            len(all_lines), block.end_line + self.config.context_lines
                        )
    ...
    def _write_side_by_side_comparison(
        self, f: TextIO, block1: Dict, block2: Dict
    ) -> None:
        ...
        f.write(
            f"**📄 {Path(block1['file_path']).name}** (lines {block1['start_line']}-{block1['end_line']})\n\n"
        )
        ...
        f.write('<div style="flex: 1;">\n\n')
        f.write(
            f"**📄 {Path(block2['file_path']).name}** (lines {block2['start_line']}-{block2['end_line']})\n\n"
        )
    ...
    def _write_sequential_blocks(self, f: TextIO, block1: Dict, block2: Dict) -> None:
        ...
        for i, block in enumerate([block1, block2], 1):
            f.write(
                f"**Block {i}:** `{Path(block['file_path']).name}` "
        ...
        if self.database:
            try:
                ...
                if block1_info or block2_info:
                    base_data["blocks"] = {"block1": block1_info, "block2": block2_info}

            except Exception as e:
                logger.debug(
                    f"Could not fetch block info for finding {finding.id}: {e}"
                )
...
def generate_markdown_report(
    ...
    database: Optional[EchoDatabase] = None,
) -> None:
...
def generate_json_report(
    scan_result: ScanResult, output_path: Path, database: Optional[EchoDatabase] = None
) -> None:
    """Generate a comprehensive JSON report."""
    # Initialize database if not provided
    if database is None:
