...
class TokenNormalizer:
    ...
    def normalize_block(self, block: CodeBlock) -> NormalizedBlock:
        ...
        logger.debug(
            f"Normalized block from {block.file_path}:{block.start_line} "
    ...
    for language, lang_blocks in blocks_by_lang.items():
        ...
        for block in lang_blocks:
            try:
                ...
                logger.warning(
                    f"Failed to normalize block from "
...
def create_diff_explanation(
    block1: NormalizedBlock, block2: NormalizedBlock
) -> Dict[str, any]:
