...
from .index import RepositoryIndexer, index_repository
from .normalize import create_diff_explanation
from .scan import DuplicateScanner, ScanResult, create_scanner
from .storage import DuplicateFinding, EchoDatabase, create_database
...
class EchoMCPServer:
    ...
    async def index_repo(
        self, root: Union[str, Path], reindex: bool = False
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await asyncio.to_thread(self._run_indexing, root_path, reindex)
            ...
            return {"started": False, "error": str(e), "stats": {}}
        finally:
            self._indexing_in_progress = False
    ...
    async def index_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            lsh_percent = (
                f"{(lsh_indexed * 100 // total_blocks) if total_blocks > 0 else 0}%"
            )
    ...
    async def scan_changed(
        ...
        try:
            ...
            if not file_paths:
                yield {"type": "info", "message": "No changed files to scan"}
                return
            ...
            for finding in result.findings:
                ...
                yield {"type": "finding", "resource": resource}
    ...
    async def scan_repo(
        ...
        try:
            logger.info(
                f"Repository scan requested (scope={scope}, budget={budget_ms}ms)"
            )

            # Run scan in thread pool to avoid blocking
            result = await asyncio.to_thread(
                self.scanner.scan_repository, self._current_repo_root, budget_ms
            )
    ...
    async def explain(self, finding_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            findings = await asyncio.to_thread(self._get_finding_by_id, finding_id)
            ...
            source_frame = await self._generate_codeframe(
                source_block.file_path, source_block.start, source_block.end
            )

            match_frame = await self._generate_codeframe(
                match_block.file_path, match_block.start, match_block.end
            )
    ...
    def _filter_by_scope(
        self, resources: List[Dict[str, Any]], scope: str
    ) -> List[Dict[str, Any]]:
    ...
    def _get_finding_by_id(self, finding_id: str) -> List[DuplicateFinding]:
        ...
        try:
            ...
            with self.database.get_session() as session:
                ...
                finding_record = (
                    session.query(FindingRecord).filter_by(id=finding_id).first()
                )
...
class MCPProtocolHandler:
    ...
    async def _handle_scan_changed(
        ...
        while True:
            try:
                ...
                if request.get("method") == "scan_changed":
                    ...
                    async for result in handler._handle_scan_changed(
                        request.get("params", {})
                    ):
                        response = {
                            "jsonrpc": "2.0",
