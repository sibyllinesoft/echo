...
class SemanticEmbedder:
    ...
    def __init__(
        self, config: EchoConfig, model_name: str = "microsoft/graphcodebert-base"
    ):
        self.config = config
    ...
    def embed_blocks(
        self, blocks: List[NormalizedBlock], batch_size: int = 16
    ) -> np.ndarray:
...
def compute_semantic_similarity(
    block1: NormalizedBlock, block2: NormalizedBlock, embedder: SemanticEmbedder
) -> float:
...
def batch_semantic_similarity(
    ...
    threshold: float = 0.83,
) -> List[Tuple[NormalizedBlock, float]]:
